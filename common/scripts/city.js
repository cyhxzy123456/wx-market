...
var city;
//子函数添加城市
export function addCitys(province,city)
{
...
//初始化地区下拉菜单
export function initialize(privinceId,cityId)
{
    //获取select元素
    ...
    for(var i=0;i<provinceOb.length;i++)
    {
        ...
        //设置province默认选项
        //$("#provincecode option[text='"+privinceId+"']").attr("selected", true);
        $("#provincecode option:contains('"+privinceId+"')").attr('selected','true'); 
        ...
        if(cityId!=undefined)
        {
          	$("#citycode option:contains('"+cityId+"')").attr('selected','true'); 
        }
        else
        { 
  ...
  //下拉列表改变事件
 export function selectchange(province,city)
    {
        delCitys(city);
        addCitys(province,city);
    }